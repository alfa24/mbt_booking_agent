...
2. **При отправке формы:**
   ...
   - Проверка localStorage на наличие пользователя
   - Если нет пользователя:
     - Запрос к API: `POST /users` с `telegram_id` и `telegram_username`
     - Сохранение полученного пользователя в localStorage и Zustand store
   - Если пользователь есть:
     - Запрос к API: `GET /users/{id}` для актуализации данных
...
1. **При загрузке:**
   - Запрос к API: `GET /bookings?user_id={current_user_id}`
...
3. **При отмене бронирования:**
   ...
   - Запрос к API: `DELETE /bookings/{id}`
...
**Примечание:** Для полного обновления всех полей используется `PUT /houses/{id}`, для частичного обновления — `PATCH /houses/{id}`.
